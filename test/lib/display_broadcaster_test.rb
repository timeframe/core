...
class DisplayBroadcasterTest < Minitest::Test
  def setup
    ...
    DisplayBroadcaster.clear_hash(:all)
  end
  ...
  def test_broadcast_if_changed_broadcasts_on_first_call
    ...
    broadcasts = track_broadcasts do
      DisplayBroadcaster.broadcast_if_changed(device)
    end
  ...
  def test_broadcast_if_changed_rebroadcasts_after_clear_hash
    ...
    broadcasts = track_broadcasts do
      DisplayBroadcaster.broadcast_if_changed(device)
      DisplayBroadcaster.clear_hash(device.id)
      DisplayBroadcaster.broadcast_if_changed(device)
    end
  ...
  def test_broadcast_all_mira_displays_broadcasts_mira_devices
  ...
  def test_broadcast_all_mira_displays_handles_errors
    Device.create!(location: test_location, name: "test_broadcast_err", model: "boox_mira_pro")

    DisplayBroadcaster.stub(:view_object_for, ->(_) { raise "boom" }) do
      # Should not raise
      DisplayBroadcaster.broadcast_all_mira_displays
    end
  ...
  def track_broadcasts(&block)
    broadcasts = []
    DisplayChannel.stub(:broadcast_to, ->(device, data) { broadcasts << {device: device, data: data} }) do
      block.call
    end
    broadcasts
  end
